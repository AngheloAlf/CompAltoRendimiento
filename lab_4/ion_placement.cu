...
__global__ void update_Qs(float *dev_Q, int ion_min_pos, int iter){
...
void wea_q_no_funca(){
    ...
    Q[poscicion_iones[0] = searchMinPos(dev_Q)] = INFINITY;
    cudaMemcpy(dev_Q, Q, GRID_SIZE*GRID_SIZE * sizeof(float), cudaMemcpyHostToDevice);

    for(int i=1; i<MAX_IONS-INITIAL_IONS; i++){
        
        update_Qs<<<(GRID_SIZE*GRID_SIZE/BLOCK_SIZE), BLOCK_SIZE>>>(dev_Q, poscicion_iones[i-1], i);
